...
class SoftNS:
    ...
    def soft_perturb(self, embeddings, importance_scores, attention_mask):
        ...
        importance_scores = torch.sigmoid(importance_scores)  # Softens the importance to the range [0, 1]
        ...
        # Ensure that the mask and attention_mask have compatible shapes
        ...
        perturbed_embeddings = embeddings * mask  # Apply the mask to perturb the embeddings
        # print(f"Mask: {mask}")
        # print(f"Embeddings before perturbation: {embeddings}")
        # print(f"Embeddings after perturbation: {perturbed_embeddings}")
        # print(f"Importance Scores: {importance_scores}")
        # print(f"Number of ones in the mask: {mask.sum()}")
    ...
    def compute(self, original_sentences, batch_size=8):
        ...
        # # Ensure the input batch size is consistent with the saliency scores batch size
        # assert inputs["input_ids"].size(0) == batch_size, f"Input batch size mismatch: {inputs['input_ids'].size(0)} vs {batch_size}"
        ...
        # perturbed_input['input_ids'] = perturbed_embeddings.argmax(dim=-1)  # Convert embeddings to token IDs for input
         # Ensure input_ids is not part of perturbed_input
        if 'input_ids' in perturbed_input:
            del perturbed_input['input_ids']
        ...
        print(f"normalized_sufficiency {normalized_sufficiency}")
...
def compute_all_soft_ns(dataset, model, tokenizer, precomputed_scores, max_len=512, batch_size=8):
    ...
    for i in range(0, num_samples, batch_size):
        ...
        # Get the batch size based on the current batch
        batch_size = len(saliency_scores)
        ...
        # normalized_tensor = (importance_scores - min_value) / (max_value - min_value)
